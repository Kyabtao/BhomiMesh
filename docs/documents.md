...
real source document is rendered inline via the unified viewer.
...
5. **Footer** — maturity dot+label (`BMCM.MATURITY`), level chip (`Core` / `Advanced` /
   `Standard` from `BMCM.LEVEL`), and **Open Spec →** linking to `doc.html?d=<file>` (URL-encoded,
   opens in a new tab)
...
- **New document:** add an entry to `BMCM.DOCS` and a corresponding fragment to `BMCM_DOCS` in
